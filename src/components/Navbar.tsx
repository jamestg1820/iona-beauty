...
export default function Navbar() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const items = collections.map((col: any) => ({
            ...
            href: `/coleccion/${col.handle}`
          }));
          setNavItems(items.slice(0, 6)); // Tomamos las primeras 6 para no saturar el menú
        }
      } catch (error) {
  ...
  return (
    <header className="w-full sticky top-0 z-50 bg-white border-b border-gray-100">
      {/* Top Banner */}
      <div className="bg-primary text-gray-800 text-[10px] py-2 text-center font-bold tracking-[0.1em] uppercase">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-shrink-0 flex items-center justify-center">
            <Link href="/" className="group flex flex-col items-center">
              ...
              <div className="w-full h-[3px] bg-black mt-1 group-hover:bg-primary transition-colors"></div>
            </Link>
          </div>
      ...
      <nav className="hidden md:flex justify-center space-x-8 pb-4">
        {navItems.map((item) => (
          <Link 
            key={item.name} 
            href={item.href} 
            className="text-[11px] font-black tracking-widest text-gray-900 hover:text-primary transition-colors"
          >
            {item.name}
          </Link>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <div className="relative w-4/5 max-w-sm bg-white h-full shadow-2xl flex flex-col overflow-y-auto animate-in slide-in-from-left duration-300">
            ...
            <div className="flex flex-col py-6 px-6 space-y-6">
              <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="text-[13px] font-black tracking-widest text-gray-900 hover:text-primary transition-colors">
                INICIO
              </Link>
              ...
              {navItems.map((item) => (
                <Link 
                  ...
                  className="text-[13px] font-black tracking-widest text-gray-900 hover:text-primary transition-colors"
                >
                  {item.name}
                </Link>
              ))}
