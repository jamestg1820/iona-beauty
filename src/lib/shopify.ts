...
export async function getProducts() {
  ...
  return response.body.data.products.edges.map(({ node }: any) => {
    ...
    return {
      ...
      image: node.images.edges[0]?.node?.url || 'https://images.unsplash.com/photo-1612817288484-6f916006741a?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
...
export async function getCollections() {
  ...
  return response.body.data.collections.edges.map(({ node }: any) => ({
    id: node.id,
    title: node.title,
    handle: node.handle,
    image: node.image?.url || 'https://images.unsplash.com/photo-1591561954557-26941169b49e?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80'
  }));
}
